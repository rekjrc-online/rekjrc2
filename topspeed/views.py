...
class Start_(LoginRequiredMixin, View):
    template_name = 'races/topspeed_start.html'
    def get(self, request, race_uuid):
        ...
        for racedriver in racedrivers:
            racedriver.run = TopSpeedRun.objects.filter(race=race, racedriver=racedriver).first()
        return render(request, self.template_name, {
            'race': race,
            'racedrivers': racedrivers })

class Race_(LoginRequiredMixin, View):
    template_name = "races/topspeed_race.html"
    ...
    def post(self, request, race_uuid, racedriver_uuid):
        ...
        racedriver = get_object_or_404(RaceDriver, uuid=racedriver_uuid)
        run = get_object_or_404(TopSpeedRun, race=race, racedriver=racedriver)
        ...
        Post.objects.create(
            ...
            content=post_text)
        return redirect('topspeed:start', race_uuid=race_uuid)
...
class Finish_(LoginRequiredMixin, View):
    def post(self, request, race_uuid, *args, **kwargs):
        ...
        runs = TopSpeedRun.objects.filter(race=race).select_related('racedriver', 'racedriver__driver', 'racedriver__build')
        if not runs.exists():
            return HttpResponseForbidden("No runs found.")
        sorted_runs = runs.order_by('-topspeed')
        longest_speed = sorted_runs.first().topspeed
        winners = [run for run in sorted_runs if run.topspeed == longest_speed]
        if len(winners) == 1:
            winner = winners[0]
