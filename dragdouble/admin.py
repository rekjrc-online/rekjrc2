...
@admin.register(DragDouble)
class DragDoubleAdmin(admin.ModelAdmin):
    list_display = ("id", "bracket", "round_number", "race_display", "model1_display", "model1_record", "model2_display", "model2_record", "winner_display")
    search_fields = (
        "race__profile__display_name",
        "model1__driver__display_name",
        "model2__driver__display_name",
        "winner__driver__display_name",
    )
    list_filter = ("race",)
    ordering = ("race__created_at", "round_number")
    ...
    def model2_display(self, obj):
        return str(obj.model2) if obj.model2 else "BYE"
