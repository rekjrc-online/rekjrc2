...
class Race_(LoginRequiredMixin, View):
    ...
    def post(self, request, race_uuid, racedriver_uuid):
        ...
        racedriver = get_object_or_404(RaceDriver, uuid=racedriver_uuid)
        run = get_object_or_404(LongJumpRun, race=race, racedriver=racedriver)
        if run.feet is not None or run.inches is not None:
...
class Finish_(LoginRequiredMixin, View):
    def post(self, request, race_uuid, *args, **kwargs):
        ...
        longest_jump = sorted_runs.first().display_distance
        winners = [run for run in sorted_runs if run.display_distance == longest_jump]
        if len(winners) == 1:
            winner = winners[0]
        ...
        for idx, run in enumerate(sorted_runs, start=1):
            distance = f"{run.display_distance}" if (run.feet is not None or run.inches is not None) else "No time"
            result_lines.append(f"{idx}. {distance} | {run.racedriver.driver} | {run.racedriver.build}")
